      ...
      <CardGlobal>
        <div class="pa-1 d-flex align-center">
          ...
          <span
            ...
            class="primary--text"
          >
            Danh sách kiến nghị đã trả lời thỏa đáng
          </span>
          <v-spacer> </v-spacer>
          <div
            ...
            <v-row class="ma-0 pb-2" align="center">
              <v-col class="col-auto py-1">
                <div style="font-size: 16px; color: red !important">
                  Tổng số kiến nghị đã trả lời thỏa đáng:
                  {{ $formatMoney({ amount: repliedAgree }) }}/
...
export default {
  ...
  methods: {
    ...
    getListRequest() {
      ...
      this.$store.dispatch('request/repliedArgee', data).then((res) => {
        if (res.data.error.code === 0) {
          // console.log(res)
      ...
      this.$store.dispatch('request/requestStatistic', data).then((res) => {
        if (res.data.error.code === 0) {
          ...
          this.repliedAgree = res.data.data.repliedAgree
        }
      })
    },
  },
