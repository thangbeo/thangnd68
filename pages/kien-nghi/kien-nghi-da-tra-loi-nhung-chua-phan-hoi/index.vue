      ...
      <CardGlobal>
        <div class="pa-1 d-flex align-center">
          ...
          <span
            ...
            v-else
          >
            Danh sách kiến nghị đã trả lời nhưng chưa phản hồi
          </span>
              ...
              <v-col class="col-auto py-1">
                <div style="font-size: 16px; color: red !important">
                  <span
                    ...
                    "
                  >
                    Tổng số kiến nghị trả lời mới
                  </span>
                  <span v-else>
                    Tổng số kiến nghị đã trả lời nhưng chưa phản hồi
                  </span>

                  :
                  {{ $formatMoney({ amount: totalApprovedRequest }) }}/{{
...
export default {
  ...
  methods: {
    ...
    getListRequest() {
      ...
      this.$store.dispatch('request/repliedNotConfirm', data).then((res) => {
        if (res.data.error.code === 0) {
      ...
      this.$store.dispatch('request/requestStatistic', data).then((res) => {
        if (res.data.error.code === 0) {
          ...
          this.totalApprovedRequest = res.data.data.repliedNotconfirm
