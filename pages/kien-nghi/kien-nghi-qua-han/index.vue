        ...
        >
            ...
            <v-row class="ma-0 pb-2" align="center">
              <v-col class="col-auto py-1">
                <div style="font-size: 16px; color: red !important">
                  Tổng số kiến nghị quá hạn:
                  {{ $formatMoney({ amount: totalOverRecord }) }}/{{
          ...
          >
            ...
            >
              ...
              <td style="cursor: pointer">
                <v-row no-gutters align="center" class="hide-text">
                  <v-col
                    :cols="
                      (data.item.seenStatus !== 'SEEN' &&
                        data.item.seenStatus !== null) ||
...
export default {
  ...
  methods: {
    ...
    getListRequest() {
      ...
      this.$store.dispatch('request/requestOverdue', data).then((res) => {
        if (res.data.error.code === 0) {
          console.log(res)
          this.listItem = res.data.data.data
          this.pageCount = res.data.data.totalPages
          this.totalOverRecord = res.data.data.totalRecords
